package com.geofahm.shahrdari.model.controller.ensani46;
...
import com.geofahm.shahrdari.model.service.ensani.Ensani46Service;
...
@RequestMapping("/ensani46")
public class Ensani46Controller {
    private final Ensani46Service service;
    private final JwtUtils jwtUtils;

    private final UsersRepository usersRepository;

    @Autowired
    public Ensani46Controller(Ensani46Service service, JwtUtils jwtUtils, UsersRepository usersRepository) {
    ...
    public ResponseEntity<ResponseModel> add(@Valid @RequestBody Ensani46 edit, HttpServletRequest httpServletRequest) {
        String jwt = httpServletRequest.getHeader("Authorization");
        String userName=jwtUtils.getUserName(jwt);
        Users admin = usersRepository.findByEmail(userName);
        if (admin.getRoles().get(0).getName().equals("admin")) {
            Ensani46 edit2 = service.save(edit);
    ...
    public ResponseEntity<ResponseModel> delete(@Valid @RequestBody Education edit,HttpServletRequest httpServletRequest) {
        ...
        if (admin.getRoles().get(0).getName().equals("admin")) {
            ...
            Ensani46 studentUni = service.findById(edit.getId());
    ...
    @PostMapping("/edit")
    //@PreAuthorize("hasAuthority('OP_ACCESS_ADMIN')")
    public ResponseEntity<ResponseModel> edit(@Valid @RequestBody Ensani46 edit, HttpServletRequest httpServletRequest) {
        ...
        if (admin.getRoles().get(0).getName().equals("admin")) {
            ...
            if (studentUni != null) {
                ...
                Ensani46 edit2 = service.edit(edit);
                List list2 = new ArrayList();
                list2.add(edit2);
                ResponseModel responseModel2 = new ResponseModel();
                responseModel2.setData(list2);
