package com.geofahm.shahrdari.model.controller.education;
...
import com.geofahm.shahrdari.model.service.education.TrainedTechnicalAndProfessionalService;
...
@RequestMapping("/trained-technical")
public class TrainedTechnicalAndProfessionalController {
    private final TrainedTechnicalAndProfessionalService service;
    private final JwtUtils jwtUtils;

    private final UsersRepository usersRepository;

    @Autowired
    public TrainedTechnicalAndProfessionalController(TrainedTechnicalAndProfessionalService service, JwtUtils jwtUtils, UsersRepository usersRepository) {
    ...
    public ResponseEntity<ResponseModel> add(@Valid @RequestBody TrainedTechnicalAndProfessional edit, HttpServletRequest httpServletRequest) {
        String jwt = httpServletRequest.getHeader("Authorization");
        String userName=jwtUtils.getUserName(jwt);
        Users admin = usersRepository.findByEmail(userName);
        if (admin.getRoles().get(0).getName().equals("admin")) {
            TrainedTechnicalAndProfessional edit2 = service.save(edit);
    ...
    public ResponseEntity<ResponseModel> delete(@Valid @RequestBody Education edit,HttpServletRequest httpServletRequest) {
        ...
        if (admin.getRoles().get(0).getName().equals("admin")) {
            ...
            TrainedTechnicalAndProfessional studentUni = service.findById(edit.getId());
            System.out.println(studentUni.getYear());
            if (studentUni != null) {
    ...
    @PostMapping("/edit")
    //@PreAuthorize("hasAuthority('OP_ACCESS_ADMIN')")
    public ResponseEntity<ResponseModel> edit(@Valid @RequestBody TrainedTechnicalAndProfessional edit, HttpServletRequest httpServletRequest) {
        ...
        if (admin.getRoles().get(0).getName().equals("admin")) {
            ...
            if (studentUni != null) {
                ...
                TrainedTechnicalAndProfessional edit2 = service.edit(edit);
                List list2 = new ArrayList();
                list2.add(edit2);
                ResponseModel responseModel2 = new ResponseModel();
                responseModel2.setData(list2);
