...
import com.geofahm.shahrdari.model.service.education.Education1715Service;
...
@RequestMapping("/education1715")
public class Education1715Controller {
    private final Education1715Service service;
    private final JwtUtils jwtUtils;

    private final UsersRepository usersRepository;

    @Autowired
    public Education1715Controller(Education1715Service service, JwtUtils jwtUtils, UsersRepository usersRepository) {
    ...
    public ResponseEntity<ResponseModel> add(@Valid @RequestBody Education1715 edit, HttpServletRequest httpServletRequest) {
        String jwt = httpServletRequest.getHeader("Authorization");
        String userName=jwtUtils.getUserName(jwt);
        Users admin = usersRepository.findByEmail(userName);
        if (admin.getRoles().get(0).getName().equals("admin")) {
            Education1715 edit2 = service.save(edit);
    ...
    public ResponseEntity<ResponseModel> delete(@Valid @RequestBody Education edit,HttpServletRequest httpServletRequest) {
        ...
        if (admin.getRoles().get(0).getName().equals("admin")) {
            ...
            Education1715 studentUni = service.findById(edit.getId());
    ...
    @PostMapping("/edit")
    //@PreAuthorize("hasAuthority('OP_ACCESS_ADMIN')")
    public ResponseEntity<ResponseModel> edit(@Valid @RequestBody Education1715 edit, HttpServletRequest httpServletRequest) {
        ...
        if (admin.getRoles().get(0).getName().equals("admin")) {
            ...
            if (studentUni != null) {
                ...
                Education1715 edit2 = service.edit(edit);
                List list2 = new ArrayList();
                list2.add(edit2);
                ResponseModel responseModel2 = new ResponseModel();
                responseModel2.setData(list2);
